...
public class ActionEngine extends TestEngineWeb {
	...
	public boolean assertElementPresent(By by, String locatorName) throws Throwable {
		...
		try {
			...
			if (!flag) {
				reporter.failureReport("AssertElementPresent", locatorName + " present in the page");
				return false;
			} else if (flag) {
	...
	public boolean mouseHoverByJavaScript(By locator, String locatorName) throws Throwable {
		...
		try {
			...
			if (!flag) {
				reporter.failureReport("MouseOver", "MouseOver action is not perform on" + locatorName);
			} else if (flag) {
	...
	public boolean waitForVisibilityOfElement(By by, String locator) throws Throwable {
		...
		try {
			...
			if (!flag) {
				reporter.failureReport("WaitForVisibilityOfElement", "Element " + locator + " is not visible");
			} else if (flag) {
				reporter.SuccessReport("WaitForVisibilityOfElement", "Element " + locator + "  is visible");
			}
		}
	}
	...
	public boolean clickUsingJavascriptExecutor(By locator, String locatorName) throws Throwable {
		...
		try {
			...
			if (!flag) {
				reporter.failureReport("Click", "Unable to clicked on" + locatorName);
				return true;
			} else {
				reporter.SuccessReport("Click", "Successfully clicked on" + locatorName);
	...
	public boolean isVisible(By locator, String locatorName) throws Throwable {
		...
		try {
			...
			if (!flag) {
				reporter.failureReport("IsVisible", locatorName + " Element is Not Visible");
			} else if (flag) {
	...
	public boolean VerifyElementPresent(By by, String locatorName, boolean expected) throws Throwable {
		...
		try {
			...
			/*
			 * if(expected == status) {
			 * this.reporter.SuccessReport("isElementPresent",
			 * "isElementPresent"); } else {
			 * this.reporter.failureReport("isElementPresent",
	...
	public boolean typeUsingJavaScriptExecutor(By locator, String testdata, String locatorName) throws Throwable {
		...
		try {
			...
			/*
			 * Driver.findElement(locator).click();
			 * Driver.findElement(locator).clear(); Thread.sleep(sleep);
			 * Driver.findElement(locator).sendKeys(testdata);
			 */
	...
	public boolean asserTitle(String title) throws Throwable {
		...
		try {
			...
			if (!flag) {
				reporter.failureReport("AsserTitle", "Page title is not matched with" + title);
				return false;
			} else if (flag) {
	...
	public boolean verify(String act, String exp, String value) throws Throwable {
		...
		if (flag == false) {
			this.reporter.failureReport("Varification", value + "are not verified");
			return flag;
		} else if (flag == true) {
			this.reporter.SuccessReport("Varification", value + " Verified Sucessfully");
			return flag;
		}
	...
	public float appLoadingTime(By locator) throws Throwable {
		...
		try {
			...
			System.out.println("The time taken for the page to load is " + timeTaken);
			flag = true;
